...
uint32_t LQR_process_speed(char *buf, uint32_t len) {
    ...
    while ((start = strstr(start, "SPD:")) != NULL) {
        if (sscanf(start, "SPD:%d,%d,POS:%d,%d", &left_v, &right_v, &left_p, &right_p) == 4) {
            left_Wheel_Speed = left_v  / 60.0f;
            left_Wheel_position = left_p;
            right_Wheel_Speed = -right_v  / 60.0f;
...
{
    char buf[64];
    int left_duty = (int)(left_sp * 1820.0f); // 将速度转换为占空比百分比
    int right_duty = (int)(right_sp * 1820.0f);
    if (left_duty > 8000) left_duty = 10000;
    if (left_duty < -8000) left_duty = -10000;
    if (right_duty > 8000) right_duty = 10000;
    if (right_duty < -8000) right_duty = -10000;
