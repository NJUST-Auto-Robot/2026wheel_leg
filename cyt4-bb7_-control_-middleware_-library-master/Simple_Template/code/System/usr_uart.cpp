...
// 串口的fifo
fifo_struct uart1_fifo, uart2_fifo, uart3_fifo, uart4_fifo,uart5_fifo;
// 串口fifo集合
fifo_struct *puart_fifo_s[6] = {&debug_uart_fifo, &uart1_fifo, &uart2_fifo,
                                &uart3_fifo, &uart4_fifo, &uart5_fifo};
// 串口fifo的buffer
uint8_t uart0_buffer[UART0_BUFSIZE], uart1_buffer[UART1_BUFSIZE],
        uart2_buffer[UART2_BUFSIZE], uart3_buffer[UART3_BUFSIZE],
        uart4_buffer[UART4_BUFSIZE], uart5_buffer[UART5_BUFSIZE];
// 串口fifo的buffer集合
uint8_t *uart_buffer_s[6] = {uart0_buffer, uart1_buffer, uart2_buffer,
                             uart3_buffer, uart4_buffer, uart5_buffer};
// 串口fifo的buffer的长度集合
uint32_t uart_buffer_len_s[6] = {UART0_BUFSIZE, UART1_BUFSIZE, UART2_BUFSIZE,
                                 UART3_BUFSIZE, UART4_BUFSIZE, UART5_BUFSIZE};
...
Send_DMA_Fun_t uart5_Send = uartSend<5>;
// 发送函数的合集
Send_DMA_Fun_t UartSendArray[6] = {uart0_Send, uart1_Send, uart2_Send,
                                  uart3_Send, uart4_Send, uart5_Send};
...
void usrUartInit(void) {
  uartSingleInit(UART_4, 420000, false, 8, CY_SCB_UART_PARITY_NONE,
                 CY_SCB_UART_STOP_BITS_1);
...
template <uint8_t uart_id> void uart_read_byte(void) {
  ...
  if (puart_fifo_s[uart_id]->init_state == true) {
    uint8_t get_byte;
    // 在中断上下文中使用非阻塞查询以避免阻塞/进入 non-root 中断
    rec_sta = uart_query_byte((uart_index_enum)uart_id, &get_byte);
    if (rec_sta) {
      // 如果接收成功
...
void (*uart5_read_byte)(void) = uart_read_byte<5>;  